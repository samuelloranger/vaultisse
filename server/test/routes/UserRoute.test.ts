...
describe("DELETE /user (account deletion)", () => {
    ...
    it("leaves the departing account's contributions in the shared library", async () => {
        ...
        const locationId = (
            await leaver.agent.post("/api/rest/location").send({ name: `Contributed Shelf ${stamp}`, description: "" })
        ).body.id;
        const authorId = (await leaver.agent.post("/api/rest/author").send({ name: `Contributed Author ${stamp}` }))
            .body.id;
        const categoryId = (
            await leaver.agent.post("/api/rest/category").send({ name: `Contributed Category ${stamp}` })
        ).body.id;
        const customerId = (
            await leaver.agent.post("/api/rest/customer").send({ name: `Contributed Customer ${stamp}` })
        ).body.id;
        const groupId = (
            await leaver.agent.post("/api/rest/customer/group").send({ name: `Contributed Group ${stamp}` })
        ).body.id;

        const stockCode = (
            await leaver.agent.post(`/api/rest/book/${bookId}/stock`).send({ status: 0, location_id: locationId })
        ).body.code;
        ...
        await leaver.agent
            .put(`/api/rest/book/${bookId}`)
            .send({ name: `Contributed Book ${stamp}`, authors: [authorId] });
        ...
        expect(
            (await stayer.agent.get("/api/rest/customer")).body.customers.some((c: any) => c.id === customerId)
        ).toBe(true);
