...
describe("admin surface access control", () => {
    it("refuses an authenticated non-admin with 403 on every endpoint", async () => {
        ...
        const { rows } = await appService
            .getDatabasePool()
            .query("SELECT disabled FROM users WHERE id = $1", [victimId]);
...
describe("PATCH /admin/users/:id", () => {
    ...
    it("promotes and demotes another account", async () => {
        ...
        expect((await other.agent.get("/api/rest/app/policy")).body.user).toMatchObject({
            role: "admin",
            isAdmin: true,
        });
        ...
        expect((await activityFor(otherId)).map((r) => r.action)).toEqual(["user_role_changed", "user_role_changed"]);
...
describe("DELETE /admin/users/:id", () => {
    ...
    it("deletes an account, keeps its books, and kills its session", async () => {
        ...
        const authorId = (await leaver.agent.post("/api/rest/author").send({ name: `Admin Deleted Author ${stamp}` }))
            .body.id;
        await leaver.agent
            .put(`/api/rest/book/${bookId}`)
            .send({ name: `Admin Deleted Book ${stamp}`, authors: [authorId] });
